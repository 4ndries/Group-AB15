...
def Same_track_color_plot(List_of_combined_trackIDs):
    ...
    #Making color list
    color_list = []
    ID_list = list(dict.fromkeys(trackID))
    for i in list(dict.fromkeys(trackID)):
        random.seed(trackID[i])
        RGB = (random.randint(0,1000)/1000,random.randint(0,10000)/10000,random.randint(0,100000)/100000)
        ...
        if trackID[i] != -1:
            ...
            if i%1 == 0:
                ...
                for ID in ID_list:
                    counter3 = counter3 +1
                    if ID == trackID[i]:
                        ax.scatter(list_x[i],list_y[i],list_z[i],color=(color_list[counter3][0],color_list[counter3][1],color_list[counter3][2]), marker='.')
    ...
    for i in range(len(trackID)):
        if trackID[i] != -1:
            ...
            #same color base on track ID
            if i%10 == 0: #<------------------------------------------------detrimes how many points it skips before plotting anouther point 
                random.seed(trackID[i])
